...
bool DITCameraTool::Algorithm::Blemish::_detectBlemish(cv::Mat image, DITCameraTool::Logger& DITLogger) const {
	...
	if (debugMode) {
		std::string exportDir = DITLogger.fileDir;
		std::string fileName = DITLogger.serialNums + "-" + DITLogger.currentDatetime + "-" + (std::string)algorithmConf["configMode"] + "-" + imageName + "-" + "fastDifferenceFilteringRawMap.jpg";
	...
	if (debugMode) {
		std::string exportDir = DITLogger.fileDir;
		std::string fileName = DITLogger.serialNums + "-" + DITLogger.currentDatetime + "-" + (std::string)algorithmConf["configMode"] + "-" + imageName + "-" + "fastDifferenceFilteringProcessed.jpg";
		std::string filePath = exportDir + fileName;
		if (debugMode) {
			_PrintVariable(exportDir);
			_PrintVariable(fileName);
	...
	if (isImageGenerate) {
		std::string exportDir = DITLogger.fileDir;
		std::string fileName = DITLogger.serialNums + "-" + DITLogger.currentDatetime + "-" + (std::string)algorithmConf["configMode"] + "-" + imageName + "-" + "fastDifferenceFiltering.jpg";
		std::string filePath = exportDir + fileName;
		if (debugMode) {
			_PrintVariable(exportDir);
			_PrintVariable(fileName);
